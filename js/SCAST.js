// a AST parser by DKZ for c# typescript javascript c++ 
var SCAST=(function(){
    ...
    function parse(input) {
        ...
        function parse_define(){
            ...
            while(!(is_punc('(')||is_punc('{')||is_op('=')||is_sp(';'))){
                r.nodes.push(input.peek())
                if(is_sp(':')||is_kw("extends")||is_kw("implements")){
                    r.is_sp=true
                }else if(is_kw("class")){
                    r.type="ClassDefine"
                }else if(is_kw("interface")){
                    r.type='InterfaceDefine'
                }else if(is_kw("namespace")){
                    r.type='NamespaceDefine'
                }else if(is_kw("void")){
                    r.return_type="void"
                }
                var n=input.next()//after next() not call peek()
                if(n.type=='id'){
                    r.ids.push(n.value)
                }
            }
            if(is_punc('(')){
                r.type="MethodDefine"
                r.value=r.ids.slice(-1)[0]
                if(!r.return_type){
                    r.return_type=r.ids[0]//can ignore
                }
                r.parameter=parse_arguments()//todo parameter
                if(is_sp(':')){//charp constructor with extended class
                    input.next();
                    r.base=input.peek()
                    input.next();
                    if(is_punc('('))r.base.parameter=parse_arguments();
                }
                if(is_punc('{'))r.body=parse_block().body
            }else if(r.type=="ClassDefine"||r.type=="InterfaceDefine"||r.type=="NamespaceDefine"){
                r.value=r.ids[0]
                if(r.is_sp){
                    r.extends=r.ids.slice(1)
                }
                if(is_punc('{'))r.body=parse_block().body
            }else if(!r.type){
                r.type="PropertyDefine"
                r.value=r.ids.slice(-1)[0]
                if(is_op('=')){
                    r.body=[parse_variable(r.value)]
                }
            }
            return r
        }
        ...
        function parse_loop(){
            ...
            if(is_punc('(')){
                ...
                console.warn("parse loop",input.peek())
                return n;
            }
        }
        function parse_if(){
            ...
            if(is_punc('(')){
                ...
                }else{//if(condition)express without block
                    r.body=[until_next_kw()]
                    return r
                }
                return r
            }
            if(is_punc('{'))r.body=parse_block().body//else
            return r
        }

        function until_next_kw(){
            let _body=[]
            var r={
                type:'Expression',
                poi:{line:input.peek().poi.line,start:input.peek().poi.start}
        ...
        function parse_variable(var_name){
            var n=input.next();
            if(is_kw('new'))return parse_new()
            else if(is_kw('function')){//todo arrow function
                var nn=input.next();
                var _r={
                    type:"FunctionDefine",
                    value:var_name,
        ...
        function parse_id(){
            ...
            if(is_punc('(')){
                ...
                return r
            }else if(is_op('=')){//Variable
                var r={
                    type:"Variable",
                    value:n.value,
                    poi:{line:n.poi.line,start:n.poi.start},
                    body:[parse_variable(n.value)]
                }
                return r;
            }else{
                return n;
        ...
        function parse_new(){
            ...
            if(is_id()){
                ...
                if(is_punc('(')){
                    ...
                    // console.warn("parse new",input.peek());
                    return nn;
                }
            }else{
                console.warn("parse new",input.peek());
        ...
        function parse_expression(){
            if (is_punc("("))return parse_arguments();
            else if (is_punc("{")) return parse_block();
            else if(is_kw("new"))return parse_new();
            else if (is_id()) return parse_id();
            else return parse_next(); 
        }
    
        function parse_statement() {
            try{
                if(is_punc(";")) skip_punc(";");
                else if(is_define()) return parse_define();
                else if(is_kw("for")||is_kw("while")||is_kw("foreach"))return parse_loop();
                else if(is_kw("if")||is_kw("else")||is_kw("switch"))return parse_if();
