...
var SCASTPY=(function(){
    ...
    function TokenStream(input){
        var current=null;
        var pykeywords=['False', 'None', 'True', 'and', 'as', 'assert', 'async', 'await', 'break', 'class', 'continue', 'def', 'del', 'elif', 'else', 'except', 'finally', 'for', 'from', 'global', 'if', 'import', 'in', 'is', 'lambda', 'nonlocal', 'not', 'or', 'pass', 'raise', 'return', 'try', 'while', 'with', 'yield']
        return {
            next  : next,
            peek  : peek,
            eof   : eof,
        ...
        function read_indentation(predicate) {
            var str = "";
            input.next()
            while (!input.eof() && predicate(input.peek())) {
                str += input.next();
        ...
        function read_next() {
            ...
            var ch = input.peek();
            start=input.poi();
            if(ch=='\n') return read_indentation(is_whitespace)
            if (ch == "#"||(ch=='/'&&input.peek(1)=='/')) {
                // skip_comment();
                // return read_next();
                return {
                    type  : "Comment",
            ...
            input.croak(`Can't handle character :${ch}(${ch.charCodeAt(0)})`);
        }
        function peek() {
            return current || (current = read_next());//after next() not call peek()
        }
        function next() {
            var tok = current;
            current = null;
    ...
    function parse(input){
        ...
        function is_sp(ch) {
            var tok = input.peek();
            return tok && tok.type == "sp" && (!ch || tok.value == ch) && tok;
        }
        function is_kw(kw) {
            var tok = input.peek();
            return tok && tok.type == "kw" && (!kw || tok.value == kw) && tok;
        ...
        function skip_split(ch) {
            if (is_sp(ch)) input.next();
            else input.croak("Expecting punctuation: \"" + ch + "\"");
        }
        function parse_next() {
            var n = input.next();
            // console.log('parse next ',n)
            return n;
        }
        function parse_toplevel() {
            ...
            return { type: "top", 'body': prog };
        }
        
        function parse_id(iscls){
            var n=input.next();
            if(is_punc('(')){
                var r={
                    type:"CallExpression",
                ...
                return r
            }else if(is_op('=')||is_op('+=')||is_op('-=')||is_op('*=')||is_op('/=')||is_op('%=')||is_op('**=')||is_op('//=')){//Variable
                input.next()
                var r={
                    type:"Variable",
                    value:n.value,
                ...
                if(iscls)r.type='PropertyDefine'
                // console.log('var',r)
                return r;
            }else if(is_sp('.')){
                input.next()
                let nn=input.peek()
                // console.log('member',nn)
                var r={
                    type:"MemberExpression",
                    value:n.value,
                    poi:{line:nn.poi.line,start:nn.poi.start},
                    body:[parse_expression()]
                }
                return r;
        ...
        function parse_block(iscls){
            ...
            var body = [];
            input.next()
            var indentation=input.peek()
            while(indentation.type!='t'||indentation.value.length===0){
                indentation=input.next()
            }
            while(input.peek()&&(input.peek().type!='t'||(input.peek().type=='t'&&input.peek().value==indentation.value))){
                var sts=parse_statement(iscls)
                sts&&body.push(sts)
            }
            return {
                type:"BlockStatement",
        ...
        function parse_function(){
            ...
            var r={
                type:'FunctionDefine',
                value:input.peek().value,
                poi:{start:input.peek().poi.start,line:input.peek().poi.line},
            }
            input.next()
        ...
        function parse_class(){
            ...
            if(is_punc('(')){
                while(!is_punc(')')){
                    let p=input.next()
                    if(is_id()){
                        r.extends.push(input.peek())
                    }
                }
                skip_punc(')')
            }
            isclass=true
            if(is_sp(':'))r.body=parse_block(true).body
            console.log('class',r,isclass)
            isclass=false
            return r
        }
        ...
        function parse_if(){
            ...
            if(is_sp(':')){r.body=parse_block().body}//else
            else{
                r.condition=parse_condition()
                if(is_sp(':')){
                    r.body=parse_block().body
                }
                return r
            }
            // console.log('parse if',r)
            return r
        }
        ...
        function parse_list(){
            ...
            var end=']'
            var body=[]
            var poi={line:n.poi.line,start:n.poi.start}
            var str=""
            while(input.peek().value!=end){
                let nn=input.next()
                if(is_punc('['))body.push(parse_list())
                if(is_punc('('))body.push(parse_tuple())
                if(is_punc('{'))body.push(parse_object())
                str+=nn.value
                body.push(nn)
            }
            input.next()
        ...
        function parse_lambda(){
            ...
            while(!is_sp(':')){
                let arg=input.next()
                argument.body.push(arg)
                argument.value+=arg.value
            }
            r.arguments=argument
            while(!is_sp(',')&&!is_punc()&&input.peek().type!='t'){
                let b=input.next()
                r.body.push(b)
            }
        ...
        function parse_expression(iscls){
            ...
            if(is_punc('('))return parse_tuple()
            if(is_punc('{'))return parse_object()
            if(is_kw("lambda"))return parse_lambda()
            if(is_kw())return parse_keyword()
            return parse_next(); 
        }
        function parse_statement(iscls) {
            try{
                if(is_kw("def")) return parse_function();
                else if(is_kw("if")||is_kw("elif")||is_kw("else")) return parse_if();
                else if(is_kw('for')||is_kw('while')) return parse_loop();
                else if(is_kw('with')) return parse_with();
                else if(is_kw('try'))return parse_try()
                else if(is_kw('except'))return parse_except()
                else if(is_kw('finally'))return parse_finally()
                else if(is_kw('class'))return parse_class()
                return parse_expression(iscls);
            }catch(err){
                var p=input.peek()
                console.warn(`parse statement ${p.type} ${p.value} at ${p.poi.line}:${p.poi.start}:${p.poi.end}`,err)
            }
        }
    }

    function getAst(code){
        var inputSteam = InputStream(code);
        var tokenSteam = TokenStream(inputSteam);
        return parse(tokenSteam);
    }
    ...
    var d3config={scastops:types,fontsize:14}
